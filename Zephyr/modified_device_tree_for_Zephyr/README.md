...
- For the Zephyr , we will be first editing the pl.dtsi and system_top.dts and then running the looper command which generates mbv32.dts , finally.
    ...
    - After the modification as mentioned at above steps, the modified device tree , `pl.dtsi`, `system_top.dts` and final device tree `mbv32.dts` are available at current directory. 
